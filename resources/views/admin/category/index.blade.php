    ...
    <div x-data="{
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Skill categories data management</p>
            </div>
            <span
        ...
        <div
            ...
            <div class="flex items-center gap-2 w-full lg:w-auto justify-between lg:justify-start">
                ...
                <form action="{{ route('admin.categories.destroy-bulk') }}" method="POST" x-show="selectedIds.length > 0" x-cloak
            ...
            {{-- Form Search & Limit --}}
            <form action="{{ route('admin.categories.index') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                {{-- Dropdown Per Page --}}
                ...
                <div class="relative w-full sm:w-64">
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.categories.index') }}" class="absolute inset-y-0 right-0 pr-2.5 flex items-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse($categories ?? [] as $category)
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition">
                                <td class="p-4">
                                    <input type="checkbox" value="{{ $category->id }}" @click="toggleRow('{{ $category->id }}')"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="p-8 text-center text-slate-400 dark:text-slate-500">
                                    @if(request('search'))
                                        No skill categories found with keyword "<span class="font-semibold text-slate-600 dark:text-slate-300">{{ request('search') }}</span>".
                                    @else
