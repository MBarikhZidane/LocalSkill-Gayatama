    ...
    <div x-data="{
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Data management for user services listing</p>
            </div>
            <span
        ...
        <div
            ...
            <div class="flex items-center gap-2 w-full lg:w-auto justify-between lg:justify-start">
                ...
                <form action="{{ route('admin.services.destroy-bulk') }}" method="POST" x-show="selectedIds.length > 0" x-cloak
                    onsubmit="return confirm('Are you sure you want to delete the selected services?');">
                    @csrf
                    @method('DELETE')
            ...
            {{-- Form Search, Status Filter & Limit --}}
            <form action="{{ route('admin.services.index') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                ...
                <div class="relative w-full sm:w-64">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search title, creator..."
                    ...
                    @if(request('search') || request('status'))
                        <a href="{{ route('admin.services.index') }}" class="absolute inset-y-0 right-0 pr-2.5 flex items-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                ...
                <table class="w-full text-left text-xs text-slate-600 dark:text-slate-300">
                    <thead
                        ...
                        <tr>
                            ...
                            <th class="p-4">Creator & Category</th>
                            <th class="p-4">Price & Estimation</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse($services ?? [] as $service)
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition">
                                ...
                                <td class="p-4">
                                    <div class="font-bold text-slate-900 dark:text-white">{{ $service->title }}</div>
                                    <div class="text-[11px] text-slate-400 line-clamp-1 mt-0.5">{{ $service->description }}</div>
                                </td>
                                <td class="p-4">
                                ...
                                <td class="p-4">
                                    ...
                                    <div class="text-slate-400 text-[11px]">
                                        {{ $service->estimated_days }} {{ Str::plural('Day', $service->estimated_days) }} Delivery
                                    </div>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="p-8 text-center text-slate-400 dark:text-slate-500">
                                    @if(request('search') || request('status'))
                                        No service data found matching the search criteria.
                                    @else
