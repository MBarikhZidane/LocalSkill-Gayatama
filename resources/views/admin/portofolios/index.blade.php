    ...
    <div x-data="{
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Data management and verification for user portfolios</p>
            </div>
            <span
        ...
        {{-- Toolbar / Action & Search & Filter Card --}}
        <div
            ...
            <div class="flex items-center gap-2 w-full lg:w-auto justify-between lg:justify-start">
                ...
                <form action="{{ route('admin.portofolios.destroy-bulk') }}" method="POST" x-show="selectedIds.length > 0" x-cloak
                    onsubmit="return confirm('Are you sure you want to delete the selected item(s)?');">
                    @csrf
                    @method('DELETE')
            ...
            {{-- Form Filter, Search & Limit --}}
            <form action="{{ route('admin.portofolios.index') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">

                {{-- Skill Filter --}}
                <div class="w-full sm:w-auto">
                    <select name="skill_id" onchange="this.form.submit()"
                        class="select select-bordered select-sm text-xs bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 w-full">
                ...
                <div class="relative w-full sm:w-64">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search user, type, evidence..."
                    ...
                    @if(request('search') || request('skill_id'))
                        <a href="{{ route('admin.portofolios.index') }}" class="absolute inset-y-0 right-0 pr-2.5 flex items-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-200" title="Reset Filters">
        ...
        <div
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse($portofolios ?? [] as $portofolio)
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition">
                                <td class="p-4">
                                    <input type="checkbox" value="{{ $portofolio->id }}" @click="toggleRow('{{ $portofolio->id }}')"
                                        :checked="selectedIds.includes('{{ $portofolio->id }}')"
                                        class="row-checkbox checkbox checkbox-xs checkbox-emerald">
                                </td>
                                <td class="p-4 font-bold text-slate-900 dark:text-white">
                                    {{ $portofolio->user->name ?? 'N/A' }}
                                </td>
                                <td class="p-4">
                                    <span class="px-2 py-1 text-xs rounded-md bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-medium">
                                        {{ $portofolio->skill->name ?? 'No Skill' }}
                                    </span>
                                </td>
                                <td class="p-4 text-slate-900 dark:text-white">
                                    {{ ucfirst($portofolio->type ?? '-') }}
                                </td>
                                <td class="p-4">
                                    @if($portofolio->evidence)
                                        <a href="{{ asset('storage/' . $portofolio->evidence) }}" target="_blank"
                                           class="inline-flex items-center gap-1.5 text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 underline font-medium">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="p-8 text-center text-slate-400 dark:text-slate-500">
                                    @if(request('search') || request('skill_id'))
                                        No portfolio data found matching the selected filters.
                                    @else
            ...
            @if(isset($portofolios) && method_exists($portofolios, 'hasPages') && $portofolios->hasPages())
