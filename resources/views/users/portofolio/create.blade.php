    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Add certificates, project results, or proof of your skills.</p>
            </div>
            <a href="{{ route('user.portofolios.index') }}"
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
            <form action="{{ route('user.portofolios.store') }}" method="POST" enctype="multipart/form-data"
                class="space-y-5">
                @csrf
                ...
                <div>
                    <label for="skill_id"
                        ...
                        Skill / Subject <span class="text-rose-500">*</span>
                    </label>
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="endorsement" {{ old('type') == 'endorsement' ? 'selected' : '' }}>Endorsement</option>
                    </select>
                    @error('type')
                        <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
        ...
        removeFile() {
            ...
            this.$refs.fileInput.value = '';
                    ...
                    <label for="evidence"
                        ...
                        Upload Evidence <span class="text-slate-400 font-normal">(PDF, Image) — Optional, max 5MB</span>
                    </label>
                    ...
                    <div @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false"
                        ...
                        <!-- Hidden File Input -->
                        <input type="file" name="evidence" id="evidence" x-ref="fileInput" accept=".pdf,image/*"
                        ...
                        <template x-if="!fileName">
                            <div>
                                <div class="flex justify-center mb-3">
                                    <div
                                        class="flex h-12 w-12 items-center justify-center rounded-full bg-emerald-100 dark:bg-emerald-950">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor"
                                            class="h-6 w-6 text-emerald-600 dark:text-emerald-400">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M12 16.5V9.75m0 0l-3 3m3-3l3 3M6.75 19.5h10.5a2.25 2.25 0 002.25-2.25v-10.5A2.25 2.25 0 0017.25 4.5H6.75A2.25 2.25 0 004.5 6.75v10.5a2.25 2.25 0 002.25 2.25z" />
                                        </svg>
                                    </div>
                                </div>
                                <p class="text-sm font-semibold text-slate-700 dark:text-slate-200">Drag file here</p>
                                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                    or <span class="font-semibold text-emerald-600 dark:text-emerald-400">click to select a file</span>
                                </p>
                                <p class="mt-2 text-[11px] text-slate-400">PDF, JPG, JPEG, PNG • Maximum 5MB</p>
                            </div>
                        </template>
                        ...
                        <template x-if="fileName">
                            <div class="flex items-center justify-between gap-4 text-left">
                                <div class="flex min-w-0 items-center gap-3">
                                    ...
                                    <div
                                        class="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-emerald-100 dark:bg-emerald-950">
