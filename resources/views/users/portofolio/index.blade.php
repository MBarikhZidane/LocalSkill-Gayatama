    ...
    <div x-data="{
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Manage your certificates, portfolio, and assessment data</p>
            </div>
        ...
        <div class="mb-4 flex flex-col lg:flex-row items-center justify-between gap-3 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div class="flex items-center gap-2 w-full lg:w-auto justify-between lg:justify-start">
                <a href="{{ route('user.portofolios.create') }}"
                    class="btn btn-sm bg-emerald-600 text-white hover:bg-emerald-700 border-0 flex items-center gap-2">
                    <i data-lucide="plus" class="w-4 h-4"></i>
                    <span>Add Portfolio</span>
                </a>

                <form action="{{ route('user.portofolios.destroy-bulk') }}" method="POST" x-show="selectedIds.length > 0" x-cloak
            ...
            {{-- Form Search & Limit --}}
            <form action="{{ route('user.portofolios.index') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                ...
                <div class="relative w-full sm:w-64">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search skill / type / status..."
                    ...
                    @if(request('search'))
                        <a href="{{ route('user.portofolios.index') }}" class="absolute inset-y-0 right-0 pr-2.5 flex items-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                        ...
                        @forelse($portofolios ?? [] as $portofolio)
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition">
                                ...
                                <td class="p-4 font-bold text-slate-900 dark:text-white">
                                    {{ $portofolio->skill->name ?? '-' }}
                                </td>
                                <td class="p-4 capitalize">
                                    <span class="px-2.5 py-1 rounded-full text-[10px] font-semibold bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                                        {{ $portofolio->type }}
                                    </span>
                                </td>
                                <td class="p-4">
                                ...
                                <td class="p-4 flex items-center justify-center gap-2">
                                    <a href="{{ route('user.portofolios.edit', $portofolio->id) }}"
                                        class="btn btn-ghost btn-xs text-slate-600 hover:text-emerald-600 dark:text-slate-400">
                                        <i data-lucide="edit-3" class="w-3.5 h-3.5"></i>
                                    </a>
                                    <form action="{{ route('user.portofolios.destroy', $portofolio->id) }}" method="POST"
                                        onsubmit="return confirm('Are you sure you want to delete this portfolio?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="p-8 text-center text-slate-400 dark:text-slate-500">
                                    @if(request('search'))
                                        No portfolio data found for keyword "<span class="font-semibold text-slate-600 dark:text-slate-300">{{ request('search') }}</span>".
                                    @else
